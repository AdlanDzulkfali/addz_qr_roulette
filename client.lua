...
AddEventHandler('addz_qr_roulette:start', function(money)
	...
	if chip >= 10 then
		...
		QRCore.Functions.Notify('You need at least 10 chips to play!', 'error')
...
AddEventHandler('addz_qr_roulette:start_game', function(action, amount)
	...
	if game_during == false then
		...
		QRCore.Functions.Notify("You have bet "..amount.." chips on "..betOnColor..". The wheel is spinning...", 'primary')
		game_during = true
		...
		if action == 'black' then
			...
			if has_value(black, randomNumber) then
				...
				SetNuiFocus(false, false)
			    ingame = false
				QRCore.Functions.Notify("Black wins! "..win.." Dollar!", 'success')
				TriggerServerEvent('addz_qr_roulette:givemoney', action, win )
			else				
				SendNUIMessage({type = 'hide_roulette'})
				SetNuiFocus(false, false)
			    ingame = false
				QRCore.Functions.Notify("Unfortunately next to it. Try it again!", 'primary')
			end
		elseif action == 'red' then
			...
			if has_value(red, randomNumber) then
				...
				QRCore.Functions.Notify("Red wins! "..win.." Dollar!", 'success')
				TriggerServerEvent('addz_qr_roulette:givemoney', action, win )
			else
				SendNUIMessage({type = 'hide_roulette'})
				SetNuiFocus(false, false)
		...
		elseif action == 'green' then
			...
			if randomNumber == 0 then
				...
				SetNuiFocus(false, false)
			    ingame = false
				QRCore.Functions.Notify("green! "..win.." Dollar!", 'success')
				TriggerServerEvent('addz_qr_roulette:givemoney', action, win )
			else
				SendNUIMessage({type = 'hide_roulette'})
				SetNuiFocus(false, false)
		...
		QRCore.Functions.Notify("The wheel is turning ...", 'primary')
...
Citizen.CreateThread(function()
	...
	for i = 1, #Config.RouletteLocation, 1 do
		...
		exports['qr-core']:createPrompt(Config.RouletteLocation[i].name, Config.RouletteLocation[i].location, 0xF3830D8E, 'Open ' .. Config.RouletteLocation[i].name, {
			type = 'client',
...
RegisterNetEvent('addz_qr_roulette:client_StartRoulette', function()
	...
	local playerMoney = QRCore.Functions.GetPlayerData().money.cash
